...
async function main(): Promise<void> {
  ...
  assert(all.length === 12, `getAgents() returns all 12 fixtures (got ${all.length})`);
  ...
  assert(healthGuard !== null && healthGuard.name === "HealthGuard", "getAgent('4001') returns HealthGuard");
  ...
  const session = await client.hire({
    ...
    config: {
      ...
      durationDays: 7,
      allowlist: [
        { protocol: "Venus", action: "adjust-collateral", market: "Venus BNB collateral", label: "smoke-test grant" },
      ],
    },
  });
  ...
  if (FULL) {
    ...
    assert(seen.includes("session_revoked"), `feed observed session_revoked (events seen: ${seen.join(", ")})`);
    assert(revokeElapsed < 5000, `feed reacted to revoke() well inside the next 5s tick (${revokeElapsed}ms)`);
  } else {
    await client.revoke(session.id);
    const next = await stream.next();
    assert(!next.done && next.value.type === "session_revoked", "getDashboard() reacts to revoke() and yields session_revoked");
