...
_NEGOTIATE = AgentSkill(
    id="negotiate",
    name="Quote the deterministic normalization canary",
    description=(
        'Send a data part {"skill": "negotiate", "task_description": "...", '
        '"terms": {"deliverables": "NFKC-normalized text plus its SHA-256 digest as JSON", '
        '"quality_standards": "identical UTF-8 input must produce identical output"}}. '
        "Input is capped at 4096 UTF-8 bytes. The fixed quote is 0.1 U. "
        "Anchor the returned wallet-signed envelope on-chain via createJob + fund, "
        "then send `notify_funded` with the job_id to request delivery."
    ),
    tags=["erc8183", "sha256", "deterministic", "bnb-chain"],
