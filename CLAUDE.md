...
- **2026-08-16 (auto-mode):** Reality-checked actual repo state against this file's claims — see `docs/AGENT-TASKS.md` for full audit. Short version: `apps/web` scaffold real, F1 landing sections built and wired, but **A0.2 design tokens were never done** (F1 shipped with a raw-hex light palette, backwards from the BUILD-PLAN dependency graph), **A0.3 fixtures/`packages/sdk` schemas don't exist**, **A0.4 demo script doesn't exist**. `docs/AGENT-TASKS.md` is now the live orchestration board (task briefs + agent ownership + reporting protocol) sitting on top of BUILD-PLAN.md. Wave 1 dispatched to `frontend-polish` (A0.2 + F1 retrofit), `proof-engine-engineer` (A0.1 sdk portion + A0.3), `hackathon-captain` (A0.4 + risk report) — check AGENT-TASKS.md for live status before starting new work.
