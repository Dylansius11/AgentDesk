...
- **Owner:** `proof-engine-engineer`
- **Status:** ⏸️ **blocked on a 30-second human step — PM-verified.** Generated 2 fresh Chapel keypairs (deployer `0x3E30AA39525ec6cD0C8054f53fCF0D7e952D4045`, attester `0xbc5a13b541c20e2C95b89bC30EA0Cb6538faeCD0`), stored **only** in `packages/contracts/.env.chapel-deploy` — I re-verified independently: `git check-ignore -v` confirms it's covered by `.gitignore:19: .env.*`, `git status` shows nothing key-shaped, `cast balance` against a live Chapel RPC confirms genuinely 0. Checked 4 faucets (official BNB Chain, Chainstack, QuickNode, faucet.trade) — all captcha- or social-auth-gated, no headless path exists; this is an honest dead end, not a misconfiguration. **Action needed:** open https://www.bnbchain.org/en/testnet-faucet, paste `0x3E30AA39525ec6cD0C8054f53fCF0D7e952D4045`, solve the captcha, claim 0.3 tBNB (~30s). Once funded, `cd packages/contracts && set -a; source .env.chapel-deploy; set +a && pnpm deploy:chapel` (drop `--verify` from the script first — no `BSCSCAN_API_KEY` this session) deploys for real — no new keys need generating, everything's staged.
